...
static LPVOID MyMalloc(DWORD dwSize)
{
	...
	/* TODO - change accces for the last page( = guard page) to PAGE_NOACCESS */
	rc = VirtualProtect(lpMem + dwNoPages * dwPageSize, dwPageSize, 
										PAGE_NOACCESS, &oldProt);
	DIE(rc == FALSE, "VirtualProtect");

	/* TODO - return a pointer to an offest in the allocated memory so that 
	 the memory area has dwSize bytes and ends before the guard page */
