...
//		MPI_Comm_rank(MPI_COMM_WORLD, &trank);
		ret = MPI_Recv(&x, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG,
			       MPI_COMM_WORLD, &stat);
		fail_unless(ret == MPI_SUCCESS);
//		fprintf(stderr,"rank %d %d recieved from %d NUMPROCS %d  SIZE %d\n",trank, rank, stat.MPI_SOURCE, NUM_PROCS,size);
	...
	ret = MPI_Finalize();
	fail_unless(ret == MPI_SUCCESS);
	fprintf(stderr, "rank %d has finished\n", rank);
	printf("success\n");
