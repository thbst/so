...
void show_snapshot(){
	for(i = 0;i < size; i++)
		...
		printf("[%p]:%x\n", stack[i].address,(unsigned int)stack[i].value/* TODO1 */);
}
...
void take_snapshot(){
	for(p = ebp; p > esp; p--){
		...
		stack[size].value   = *p; //TODO1: set the value at this address
		size++;
	}
}
...
void f2(mytype x, mytype y, mytype z){
	...
	/* TODO2: use v so that when function 'f2' returns, the 'show_message' function executes */
